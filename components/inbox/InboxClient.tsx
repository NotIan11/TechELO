    ...
    <div className="space-y-4">
      ...
      {items.length === 0 ? (
        <EmptyState
          ...
          description="Challenges, results and ledgers land here."
          action={
            <>
              <Button href="/matches/new">Challenge someone</Button>
              <Button href="/poker/sessions/new" variant="secondary">
                Start a game
              </Button>
            </>
          }
          ...
          if (item.kind === 'match') {
            ...
            const runConfirmed = () => {
              ...
              if (confirming.action === 'accept') {
                callApi(item.id, '/api/matches/accept-start', { match_id: match.id }, () => 'Challenge accepted. Report the result here when you finish.', { href: `/matches/${match.id}`, label: 'View match' })
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-white">
                      {isAcceptStart ? (
                        ...
                        <>
                          Report the result of your {gameLabel(match.game_type).toLowerCase()} match with <span className="font-semibold">{opponent?.display_name}</span>
                        </>
                      )}
                    </p>
                    <p className="mt-1 flex items-center gap-2 text-xs text-zinc-500">
                      <GameIcon game={match.game_type} size="xs" />
                      <TimeAgo date={match.created_at} />
                      <span>·</span>
                      <TextLink href={`/matches/${match.id}`} muted className="text-xs">
                        details
                      </TextLink>
                    </p>
                    {!isConfirmingThis ? (
                      <div className="mt-3 flex flex-wrap gap-2">
                        {isAcceptStart ? (
                          <>
                            <Button size="sm" disabled={!!loading} onClick={() => setConfirming({ itemId: item.id, action: 'accept' })} type="button">
                              Accept
                            </Button>
                            <Button size="sm" variant="danger" disabled={!!loading} onClick={() => setConfirming({ itemId: item.id, action: 'decline' })} type="button">
                              Decline
                            </Button>
                          </>
                        ) : (
                          <>
                            <Button size="sm" disabled={!!loading} onClick={() => setConfirming({ itemId: item.id, action: 'report', winnerId: currentUserId })} type="button">
                              I won
                            </Button>
                            <Button size="sm" variant="secondary" disabled={!!loading} onClick={() => setConfirming({ itemId: item.id, action: 'report', winnerId: opponent?.id ?? '' })} type="button">
                      ...
                      <ConfirmStrip
                        inline
                        busy={busy}
                        tone={confirming.action === 'decline' ? 'danger' : 'primary'}
            ...
            return (
              <div key={item.id} className="card animate-fade-in p-5">
                <div className="flex items-start gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-white">
                      <span className="font-semibold">{session.host?.display_name ?? 'Someone'}</span> logged <span className="font-semibold">{title}</span>.{' '}
                      {isTourney ? (
                        <>
                        ...
                        <>
                          ...
                          <span className="text-zinc-400">
                            {' '}
                            (in {formatCents(entry.buy_in_cents, { compact: true })}, out {formatCents(entry.cash_out_cents ?? 0, { compact: true })})
                          </span>
                        </>
                      )}
                    </p>
                    <p className="mt-1 flex items-center gap-2 text-xs text-zinc-500">
                      <GameIcon game="poker" size="xs" />
                      <TimeAgo date={item.sortAt} />
                      <span>·</span>
                      <TextLink href={`/poker/sessions/${session.id}`} muted className="text-xs">
                        full ledger
                      </TextLink>
                    </p>
                    {!isConfirmingThis ? (
                      <div className="mt-3 flex flex-wrap gap-2">
                        ...
                        <Button size="sm" variant="danger" disabled={!!loading} onClick={() => setConfirming({ itemId: item.id, action: 'poker_dispute' })} type="button">
                          Dispute
                        </Button>
                      </div>
                    ) : (
                      ...
                      >
                        <textarea className="input min-h-[64px]" placeholder="e.g. I cashed out $60, not $40" value={reason} maxLength={500} onChange={(e) => setReason(e.target.value)} />
                      </ConfirmStrip>
                    )}
                  </div>
                </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-white">
                    {disputes.length === 1 ? (
                      <>
                        <span className="font-semibold">{disputes[0].user?.display_name ?? 'A player'}</span> disputed your ledger for <span className="font-semibold">{sessionTitle(session)}</span>
                      </>
                    ) : (
                      <>
                  ...
                  <p className="mt-1 flex items-center gap-2 text-xs text-zinc-500">
                    <GameIcon game="poker" size="xs" />
                    <TimeAgo date={item.sortAt} />
                    <span>· still counts until you edit or void it</span>
                  </p>
                  <div className="mt-3 flex flex-wrap gap-2">
